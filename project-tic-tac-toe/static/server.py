...
async def join_room(sid, data):
    ...
    if room_id not in rooms:
        await sio.emit(
            ...
            {
                'message': (
                    f'Không tìm thấy phòng {room_id}!'
                )
            },
            to=sid,
        )
...
async def make_move(sid, data):
    ...
    player_index, player = find_player(
        room,
        sid,
    )
...
async def restart_game(sid, data):
    ...
    if len(room['players']) < 2:
        await sio.emit(
            ...
            {
                'message': (
                    'Cần đủ 2 người chơi để chơi lại!'
                )
            },
            to=sid,
        )
    ...
    # Keep the original project's replay behavior:
    # swap X/O.
    for player in room['players']:
        player['symbol'] = (
            'O'
    ...
    await sio.emit(
        'game_restarted',
        {
            'message': (
                f'🔄 Chơi lại! '
                f'{first_player["name"]} '
                f'({first_player["symbol"]}) đi trước'
            ),
            'current_player': (
                first_player['symbol']
            ),
